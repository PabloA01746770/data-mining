...
```{r}
NOMBRE_COMPLETO = "PABLO VILLAVICECNIO PICAZO"
DIGITOS_MATRICULA = 01746770
```
...
- support (mínimo) (**NOTE-12**): 0.8
- confidence (mínimo) (**NOTE-13**):0.1
- items (máximo) (**NOTE-14**):1 none FALSE
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): No eciste una realacion de pañales con otro articulo
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque las metricas de support y confidence excluyen a la regla
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**):Es representar cada una de las sesiones del dataset, y su proposito es distinguir el numero de sesiones que los usuarios tienen en Spotify para asi poder hacer recomendaciones al usuario 
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Distinguir que canciones escuchan los usuarios y con ello hacer recomendaciones.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Si, con ello se visualiza el comportanmiento de los usuarios en Spotify al momento de escuchar musica, se hacen asocianiones relacionando los gustos a traves de canciones saltadas, pausas, repeticiones, entre otras.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**):t_bacf06d3-9185-4183-84ea-ff0db51475ce t_5718ab08-3a15-4d3f-9e63-42b2f6805e31 t_8c4d29b1-e0bf-464c-88f7-ac19240cbba0  y frecuencia 884 
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 20
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): los usuarios tienen 14.59 en promedio  
...
```{r}
try(
  plot(rules, main=paste0("Elaborado por:  Pablo Villavicencio Picazo", "(",1746770, ")" ))
)
